...
// If the tenant has an OIDC configuration and an OIDC provider is wired,
// returns the OIDC provider. Otherwise returns the local provider.
func (r *Resolver) Resolve(ctx context.Context, tenantID uuid.UUID) (Provider, error) {
	if r.oidcConfig != nil && r.oidc != nil {
...
func (r *Resolver) Authenticate(ctx context.Context, req AuthRequest) (*AuthResult, error) {
	// If OIDC fields are present and an OIDC provider is wired, use it directly.
	if req.AuthorizationCode != "" && r.oidc != nil {
