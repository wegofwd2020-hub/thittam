...
var PublicMethods = map[string]string{
	...
	// Service-to-service. Grants nothing: ValidateToken only confirms whether a
	// token is currently valid, and the calling service authorizes against its
	// own verified caller. Residual risk is a verification oracle — anyone who
	// reaches iam's port can probe whether a given token is still live.
