...
// ImpersonationStore logs impersonation sessions.
type ImpersonationStore interface {
	// LogImpersonation records an impersonation session.
	LogImpersonation(ctx context.Context, req ImpersonationRequest, expiresAt interface{}) (uuid.UUID, error)
}
