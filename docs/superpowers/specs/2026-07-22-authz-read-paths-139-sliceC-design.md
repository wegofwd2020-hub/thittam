...
Fourteen read RPCs across three services return tenant business data to any authenticated member, with no permission check:
...
- **reporting-analytics (5):** `GetReportDefinition`, `ListReportDefinitions`, `GetExpenseFacts`, `GetBudgetFacts`, `GetDashboardSummary`
...
### 3.2 reporting-analytics — full authorization wiring
...
Then gate the five RPCs on `"report:read"`. reporting scopes the tenant with `tenant.IDFromContext` (`GetExpenseFacts`, `handler.go:48`), so the gate goes after that `if !ok` block, before any parse.
